...
public class Piece {
    ...
    private void initWidthHeight() {
        ...
        height = 0;
        for(TPoint pt : body){
            width = Math.max(width, pt.x+1);
            height = Math.max(height, pt.y+1);
        }
    }

    /* at first you have to calc width, because skirt's length is 'width' */
    ...
    public boolean equals(Object obj) {
        ...
        Set<TPoint> thisSet = new TreeSet<>();
        for(TPoint tp : body)
            thisSet.add(tp);
        Set<TPoint> otherSet = new TreeSet<>();
        for(TPoint tp : other.getBody())
            otherSet.add(tp);
